...
esp_err_t rmt_uart_init(rmt_uart_port_t uart_num, const rmt_uart_config_t* rmt_uart_config)
{
    ...
    memcpy(&rmt_uart_contex[uart_num].rmt_uart_config, rmt_uart_config, sizeof(rmt_uart_config_t));

    ESP_LOGI(TAG, "baud=%d rmt_div=%d rmt_tick=%d bit_len=%d", rmt_uart_config->baud_rate, RMT_DIV, RMT_TICK, bit_len);
    ...
    if (rmt_uart_config->mode == RMT_UART_MODE_TX_ONLY || rmt_uart_config->mode == RMT_UART_MODE_TX_RX)
    {
        ...
        rmt_uart_contex[uart_num].rmt_config_tx = rmt_config_tx;
                ...
                rmt_uart_contex[uart_num].rmt_uart_contex_tx.items = heap_caps_calloc(1, rmt_uart_config->buffer_size, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
        #else
                rmt_uart_contex[uart_num].rmt_uart_contex_tx.items = calloc(rmt_uart_config->tx_items_buffer_size, sizeof(rmt_item32_t));
        #endif
    }
